...
def index(request):
    if 'all_gold' not in request.session or 'activities' not in request.session:
        request.session['all_gold'] = 0
...
def process_money(request):
    if request.method == "POST":
        if request.POST['button'] == 'farm':
            ...
            request.session['activities'].append('You earned ' + str(gold) + ' golds from the ' + request.POST['button'] + ' ' + '(' + str(datetime.now().strftime("%Y-%m-%d %H:%M")) + ')')
